# -*- coding: utf-8 -*-
"""v10：强度比形式——Y=X·矛盾强度，Ylimit=X·(1−军费)·(基准+福利)；危机=矛盾强度>福利强度"""
...
def run(df, P, use_gdp):
    ...
    gini_n=gini/100
    # 矛盾强度 = 失业 + 贫富 + 危机(股崩) + Z(创新负=缓解)
    contra = P["wu"]*u_norm + P["wg"]*gini_n + P["wc"]*crisis - P["kZ"]*fZ
    contra = np.clip(contra, 0, 1.5)
    # 福利强度 = 教育(代理医疗/义务教育)
    welfare = P["we"]*edu_n
    # Y = X·矛盾强度； Ylimit = X·(1−军费)·(基准+福利)
    Y = Xn * contra
    Yl = Xn * (1-mil) * (P["w0"] + welfare)
...
P = dict(wu=0.6,wg=0.5,wc=0.7,kZ=0.3,we=0.5,w0=0.35,decay=0.5)
def load_in(name):
