# -*- coding: utf-8 -*-
"""v7 水平式 Y：Y = a1·Xn + a2·失业 + b1·Gini + kZ·Z(创新)，金融压力作为 Y 的水平成分→Y涨触碰Ylimit→危机"""
...
def run(df, P, use_gdp):
    ...
    fZ=sign_log(Z)
    # 金融压力(水平量)：失业率↑ + 股崩(用市值/GDP 偏离长期均值)
    ...
    # 水平式 Y（正确因果：Y 随 X/失业/股崩 上升，Z 缓解）
    Y = P["a1"]*Xn + P["a2"]*u_norm + P["a3"]*crash + P["b1"]*(gini/100) + P["kZ"]*fZ
    t=np.arange(len(df))
...
P = dict(a1=1.0, a2=0.8, a3=0.5, b1=0.5, kZ=0.3, kL=1.6, beta=0.6, b0=0.1, bG=0.003, Y0=1.0)
