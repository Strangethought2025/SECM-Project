# -*- coding: utf-8 -*-
"""v9：Ylimit = (X−军费) × (1+福利系数)；福利=教育+寿命代理；水平式Y+持续危机强度"""
...
def run(df, P, use_gdp):
    ...
    life = df["Life"].to_numpy(float) if "Life" in df.columns else np.full(len(df), 70.0)
    ...
    crash=np.clip(-mg,0,1); crisis=np.zeros(len(df))
    for t in range(1,len(df)): crisis[t]=P["decay"]*crisis[t-1]+crash[t]
    # 福利系数：教育(入学率) + 寿命(代理医疗) 的合成
    life_n = (life-life.min())/(life.max()-life.min()+1e-6)
    welfare = 0.6*edu_n + 0.4*life_n
    # Ylimit = (X − 军费) × (1 + 福利)
    t=np.arange(len(df))
    Yl = P["kL"]*(Xn - mil*Xn) * (1 + P["kW"]*welfare)
    # 水平式 Y + 危机正反馈
    Y=np.zeros(len(df)); Y[0]=P["Y0"]
    for t in range(1,len(df)):
        Y[t]=P["a1"]*Xn[t]+P["a2"]*u_norm[t]+P["a3"]*crisis[t]+P["b1"]*(gini[t]/100)+P["kZ"]*fZ[t]
        if Y[t-1]>Yl[t-1]: Y[t]+=P["fb"]*(Y[t-1]-Yl[t-1])
...
P = dict(a1=1.0,a2=0.8,a3=0.6,b1=0.5,kZ=0.3,kL=1.8,kW=0.8,Y0=1.0,decay=0.5,fb=0.2)
def load_in(name):
